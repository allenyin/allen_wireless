...
#define A1_AGCB		    (A1_IIRSTARTA + A1_IIR)      //19 
#define A1_IIRSTARTB	(A1_AGCB + A1_AGC + A1_LMS)  //30
#define A1_TEMPA		(A1_IIRSTARTB + A1_IIR)      //38
#define A1_APERTUREA	(A1_TEMPA + A1_TEMPLATE)     //54
#define A1_TEMPB		(A1_APERTUREA + A1_APERTURE) //56
#define A1_APERTUREB	(A1_TEMPB + A1_TEMPLATE)     //72
#define A1_STRIDE		(A1_APERTUREB + A1_APERTURE) //74 
...
/* 
  1 sample + 1 integrator mean + 1 AGC gain + 1 saturated AGC out + 1 AGC-out + 
  IIR related: x0(n-1), x0(n-2), y1(n-1), y1(n-2), y2(n-1), y2(n-2)
  = 11 16-bit words per channel.
  11 32-bit words per 2 channels --> 22 32-bit words per group of 4.
*/
#define	W1_STRIDE		11
